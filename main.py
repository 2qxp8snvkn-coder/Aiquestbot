...
class QuestSelect(discord.ui.Select):
 def __init__(self):
 options=[
 discord.SelectOption(label=quests_available["quest1"], value="quest1", emoji="🌟"),
 discord.SelectOption(label=quests_available["quest2"], value="quest2", emoji="💪"),
 discord.SelectOption(label=quests_available["quest3"], value="quest3", emoji="⚔️")
 ]
 super().__init__(placeholder='Select a quest...', min_values=1, max_values=len(options), options=options)
 
 async def callback(self, interaction: discord.Interaction):
 selected_quest = self.values[0]
 
 user_data = c.execute("SELECT  FROM tokens WHERE owner=?", (interaction.user.name,))
 ...
 c.execute("UPDATE tokens SET balance=balance-1 WHERE owner=?", (interaction.user.name,))
 conn.commit()
 
 await interaction.response.send_message(f"Quest '{selected_quest}' executed successfully!")
 else:
 await interaction.response.send_message("Insufficient credits or invalid request.")
...
async def do_quests(ctx):
 ...
 await ctx.send('Which quest would you like to execute?', view=view)
